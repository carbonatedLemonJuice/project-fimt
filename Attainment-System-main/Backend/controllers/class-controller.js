...
const sclassCreate = async (req, res) => {
    try {
        ...
        res.status(500).json({ message: err.message || "Error creating class" });
    }
};
...
const sclassList = async (req, res) => {
    try {
        ...
        res.status(500).json({ message: err.message || "Error fetching classes" });
    }
};
...
const getSclassDetail = async (req, res) => {
    try {
        ...
        res.status(500).json({ message: err.message || "Error fetching class details" });
    }
}
...
const getSclassStudents = async (req, res) => {
    try {
        ...
        res.status(500).json({ message: err.message || "Error fetching class students" });
    }
}
...
const deleteSclasses = async (req, res) => {
    try {
        ...
        res.status(500).json({ message: error.message || "Error deleting classes" });
    }
}
