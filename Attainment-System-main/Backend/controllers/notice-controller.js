...
const noticeCreate = async (req, res) => {
    try {
        ...
        res.status(500).json({ message: err.message || "Error creating notice" });
    }
};
...
const deleteNotice = async (req, res) => {
    try {
        ...
        res.status(500).json({ message: error.message || "Error deleting notice" });
    }
}
...
const deleteNotices = async (req, res) => {
    try {
        ...
        res.status(500).json({ message: error.message || "Error deleting notices" });
    }
}
